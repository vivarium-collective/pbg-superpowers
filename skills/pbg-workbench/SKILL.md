...
> `reports/index.html`). `/pbg-workbench` manages the *interactive* workbench.
...
**To use it:** run one workbench (`/pbg-workbench start` from any workspace, or
...
- **`/pbg-workbench start`** — pick a free port (prefer 8765), launch
  ...
  browser.
- **`/pbg-workbench stop`** — read the PID file, SIGTERM, wait up to 5 s, escalate
  to SIGKILL if needed, clear state files.
- **`/pbg-workbench status`** — `alive` / `stale` / `not-running`. Probes both the
  PID (`kill -0`) and the HTTP endpoint to distinguish a wedged process from a
  healthy one.
- **`/pbg-workbench open [--investigation SLUG]`** — open the workbench URL in the
...
- **`/pbg-workbench restart`** — `stop` then `start`. Useful after a code change in
...
server. Running `/pbg-workbench start` from two worktrees concurrently is
...
any stage's lifecycle. Invoke `/pbg-workbench` only when the user wants to look at
