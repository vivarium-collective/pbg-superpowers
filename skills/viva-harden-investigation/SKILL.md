...
| Failing / partial report-card gate with a real divergence | **ROOT-CAUSE it first — REQUIRED: superpowers:systematic-debugging.** No fix, no added rigor, until you can state the cause |
...
`origin/main` (REQUIRED: superpowers:using-git-worktrees). Commit early and often so progress is
externally visible. When merging, **cherry-pick onto current origin** — a worktree created off a
stale base reverts newer work on a naive merge.
...
"harden the flagship" into one root-cause that hardens two investigations.
