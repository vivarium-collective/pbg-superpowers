...
| Form | What it does |
|---|---|
| `/pbg-catalog`  (no args) | Same as `/pbg-catalog list`. |
| `/pbg-catalog list` | One-screen workspace summary — composites, studies, registry, installed modules, dirty count. Fetches `/api/workspace-manifest`. |
| `/pbg-catalog install <pkg>` | Add a curated `pbg-*` package, install it into the workspace `.venv`, edit `pyproject.toml`, refresh the registry. Wraps `/api/catalog-install`. |
| `/pbg-catalog uninstall <pkg>` | Inverse — remove an installed catalog module. Wraps `/api/catalog-uninstall`. |
...
If `dirty_count > 0`, hint that `/pbg-status` shows the full paths.

## install

```bash
NAME="${1:-}"
[ -n "$NAME" ] || { echo "Usage: /pbg-catalog install <package-name>"; exit 1; }
...
```bash
NAME="${1:-}"
[ -n "$NAME" ] || { echo "Usage: /pbg-catalog uninstall <package-name>"; exit 1; }
...
```text
/pbg-catalog                       # same as list
/pbg-catalog list
/pbg-catalog install spatio-flux
/pbg-catalog uninstall spatio-flux
```
