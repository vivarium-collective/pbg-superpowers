...
Use a consistent, distinguishable palette across a study's charts — pick a
small set of colors up front and reuse them by series/condition rather than
re-deriving conventions ad hoc per script.
...
Generated by /viva-viz on <date> from request '<viz-name>'.
"""
...
- File must be self-contained — only import from `viva_superpowers`,
...
| `as_visualization` | `viva_superpowers.visualization` | `Visualization` subclass (HTML-emitting Step) |
...
Generated by /viva-viz on 2026-05-12 from request 'dna-a-trajectory'.
"""
from __future__ import annotations
import html as _html, json
from viva_superpowers.visualization import as_visualization
