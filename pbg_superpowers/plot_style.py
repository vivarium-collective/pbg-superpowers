"""Framework-wide house plotting style for pbg investigations.

This module encodes the chart conventions that an expert reviewer repeatedly
re-requested across pbg investigations, so future figures stop re-incurring that
friction. The conventions, which apply to any pbg investigation:
...
It centralizes the minutes-stitching + subtitle-wrapping + house-layout logic so
every pbg investigation inherits the same expert-approved chart conventions
instead of re-deriving them per-workspace.
...
# Shared palette — neutral grey + a small categorical set.
PALETTE = {
    "grey": "#94a3b8",
    "green": "#16a34a",
    "red": "#dc2626",
...
def house_layout(fig, title: str, subtitle: str, height: int = 480):
    ...
    line count so multi-line subtitles don't crowd the plot. Returns the figure
    for chaining."""
    sub = wrap(subtitle)
