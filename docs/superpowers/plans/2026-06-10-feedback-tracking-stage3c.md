...
- Modify (dashboard): `vivarium_dashboard/server.py` (`_collect_study_feedback` → tracked, or a new study-detail field) + `static/study-detail.js` (+/or `templates/study-detail.html`) for the render. Test: `tests/` mirror.
...
## Task 2: Surface on study-detail (vivarium-dashboard, render only)
...
- Dashboard `main` may be checked out in a worktree (`vivarium-dashboard-pdmp`); create the branch off `origin/main` in the main `vivarium-dashboard` checkout (a new branch != main, so it won't conflict with the worktree).
