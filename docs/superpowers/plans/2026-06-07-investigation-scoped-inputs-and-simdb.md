...
**vivarium-workbench**
- Modify `vivarium_workbench/lib/workspace_paths.py` (vendored `inputs_dir`), create `lib/runs_index.py` (vendored), drift-guard tests.
- Modify `vivarium_workbench/server.py` — `GET /api/iset/<slug>/inputs`, `GET /api/simulations`; remove global inputs route.
- Modify `vivarium_workbench/static/walkthrough.js` + `templates/index.html.j2` — Inputs into the per-investigation rail; SimulationsDB tagged table.
...
- [ ] **Step 4: Run — PASS.**
- [ ] **Step 5: Mirror** the method verbatim into `vivarium_workbench/lib/workspace_paths.py` and confirm that repo's `workspace_paths` drift-guard test still passes (`vivarium-workbench$ .venv/bin/python -m pytest tests/test_workspace_paths.py -q`).
...
## Task 3: Scoped inputs endpoint (vivarium-workbench, TDD)

**Files:** Modify `vivarium_workbench/server.py`; create vendored `vivarium_workbench/lib/investigation_inputs.py` (mirror, drift-guarded); Test `tests/test_iset_inputs.py`.

- [ ] **Step 1:** Vendor `investigation_inputs.py` into `vivarium_workbench/lib/` (byte-faithful function body) + a drift-guard test like the `viz_freshness` mirror.
...
## Task 4: Inputs tab — investigation-first sections (vivarium-workbench)
...
- [ ] **Step 3:** `node --check vivarium_workbench/static/walkthrough.js`. Manual verify: load an investigation → Inputs appears in its rail → shows its datasets/refs.
...
## Task 8: `/api/simulations` endpoint (vivarium-workbench, TDD)

**Files:** create vendored `vivarium_workbench/lib/runs_index.py` (mirror + drift guard); modify `server.py`; Test `tests/test_simulations_endpoint.py`.

- [ ] **Step 1:** Vendor `runs_index.py` into `vivarium_workbench/lib/` (byte-faithful) + drift-guard test.
...
## Task 9: SimulationsDB tagged table (vivarium-workbench)
