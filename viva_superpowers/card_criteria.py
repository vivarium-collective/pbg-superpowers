...
Generic (no organism/sim_data imports); the canonical home is ``viva_superpowers``
so any workspace's cards share one grader. The verdict vocabulary is sourced from
``viva_superpowers.test_vocab`` (the single home for CANONICAL/COLOR/GLYPH/RANK).
"""
...
from viva_superpowers.test_vocab import CANONICAL as VERDICTS
