"""DraftProcess — a placeholder / draft process (framework).
...
topology (which process connects which stores) be designed and reviewed in the
workbench *before* anyone commits to a mechanism.
...
Because it is a ``Process`` subclass defined at module scope, a workspace's
``build_core()`` (which walks the package and registers every Process/Step)
auto-registers it. It therefore shows up in the dashboard's **Modules ->
Processes** registry tab *by name*, with its ports, and — because ``describe()``
renders the whole contract prefixed ``DRAFT —`` — its contract text and a DRAFT
marker, with **no workbench code change** required.
...
#: Contract fields, in the order ``describe()`` renders them.
...
class DraftProcess(Process):
    ...
    :func:`draft_process` decorator). The registry introspects the class
    uninitialized — it calls ``inputs()``/``outputs()`` with the *class* as
    ``self`` and ``describe()`` on an ``__new__``'d instance — so every method
    here reads class attributes and never requires a configured instance.
    """
    ...
    #: other workspace processes, e.g. ``FTUPathTableStep.contract``).
    ...
    def describe(self) -> str:
        ...
        The registry surfaces this as the process's description, so the whole
        contract (and the DRAFT marker) is visible in the Processes tab card.
        """
...
def _render_contract(label: str, contract: Dict[str, Any]) -> str:
    ...
    A single line (fields joined by "·") so it stays readable in the registry
