...
# NOTE: the plugin used to pre-render the workspace SPA here (importing
# vivarium_workbench.lib.report.render_dashboard) before serving, to avoid
# showing the bootstrap placeholder. That is now the workbench's job: its
# `serve` command renders the SPA on startup (render_dashboard(..., write_all=True)
# in vivarium_workbench/cli.py). Dropping the pre-render removes the plugin's only
# reverse import of vivarium_workbench — the plugin drives the workbench, it does
# not import it (target: viva-superpowers = skills, workbench = compute).
...
def start(workspace: Path, port: int | None = None,
    ...
    # F-friction #1: reports/index.html must be the rendered SPA, not the
    # pbg-template bootstrap placeholder. The vivarium-workbench `serve` command
    # renders it on startup (render_dashboard(..., write_all=True)), so we no
    # longer pre-render here — that avoided importing vivarium_workbench from the
